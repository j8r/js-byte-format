export default class ByteEncoder {
  ...
  _movePosInBuffer(length) {
    ...
    if (this.pos > this.buffer.byteLength) {
      const newBuffer = new ArrayBuffer(this.nextPowerOfTwo(this.pos))
  ...
  writeInt8(number) {
    const pos = this._numberCurrentPos(Int8Array)
    this.dataView.setInt8(pos, number)
    return this
  }

  writeUint8(number) {
    const pos = this._numberCurrentPos(Uint8Array)
    this.dataView.setUint8(pos, number)
    return this
  }

  writeInt16(number) {
    const pos = this._numberCurrentPos(Uint16Array)
  ...
  writeInt32(number) {
    const pos = this._numberCurrentPos(Int32Array)
    this.dataView.setInt32(pos, number, this.littleEndian)
    return this
  }

  writeUint32(number) {
    const pos = this._numberCurrentPos(Uint32Array)
    this.dataView.setUint32(pos, number, this.littleEndian)
    return this
  }

  writeFloat32(number) {
    const pos = this._numberCurrentPos(Float32Array)
    this.dataView.setFloat32(pos, number, this.littleEndian)
    return this
  }

  writeFloat64(number) {
    const pos = this._numberCurrentPos(Float64Array)
  ...
  unsafeWriteString(string) {
    ...
    const uint8Array = ByteEncoder.textEncoder.encode(string)
    const currentPos = this._movePosInBuffer(uint8Array.byteLength)
  ...
  writeString(string) {
    const index = string.indexOf(this.byteDelimiter)
    if (index > 0)
  ...
  writeBool(bool) {
    const pos = this._movePosInBuffer(1)
    this.dataView.setInt8(pos, bool ? 1 : 0)
    return this
  }
