...
export default class ByteDecoder {
  ...
  readString() {
    ...
    let byteDelimiterOffset = this.bytes.indexOf(this.byteDelimiter, this.pos) - this.pos
    let currentPos = this._movePosInBuffer(byteDelimiterOffset + 1)
    return ByteDecoder.textDecoder.decode(new Uint8Array(this.buffer, currentPos, byteDelimiterOffset))
  }
  
  readBool() {
    let int = this.dataView.getInt8(this._numberCurrentPos(Int8Array))
    if (int === 0)
      return false
    else if (int === 1)
      return true
